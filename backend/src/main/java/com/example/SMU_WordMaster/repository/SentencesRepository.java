...
public interface SentencesRepository extends JpaRepository<Sentences, Long> {
    ...
    List<Sentences> findByUsersAndWord(Users userEntity, Word wordEntity);

    // 특정 사용자와 단어 조합으로 저장된 예문 중 가장 오래된 예문 엔티티 조회
    Optional<Sentences> findTopByUsersAndWordOrderBySentenceId(Users userEntity, Word wordEntity);

    // 특정 사용자가 생성한 모든 예문 리스트 조회
    List<Sentences> findByUsers(Users userEntity);

    int countByUsersAndWord(Users userEntity, Word wordEntity);
}
