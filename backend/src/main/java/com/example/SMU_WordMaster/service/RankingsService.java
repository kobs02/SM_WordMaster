...
public class RankingsService {
    ...
    // 회원가입 직후 사용자의 랭킹 엔티티를 생성하는 서비스 메서드
    public void createRankingEntity(String email) {
        ...
        catch (Exception e) { throw new RankingEntitySaveFailedException(email, e); }
    }

    // 경험치 및 랭킹레벨 업데이트하는 서비스 메서드
    public void updateRanking(String email, List<Level> wordLevelList) {
        Users userEntity = utils.getUserEntity(email);
        ...
        catch (Exception e) { throw new ExpUpdateFailedException(email, e); }
    }

    // 본인 랭킹 + 전체 랭킹 조회하는 서비스 메서드
    public List<RankingsResponseDto> getRankingsList(String email) {
        Users userEntity = utils.getUserEntity(email);
        ...
        try {
            // 본인 랭킹 조회
            Optional<Rankings> rankingEntity = rankingsRepository.findByUsers(userEntity);
            result = utils.getRankingsList(result, rankingEntity);
